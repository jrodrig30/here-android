...
public class ReportAttendanceActivity extends ActionBarActivity {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.menu_report_attendance, menu);
        return true;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_generate_report_attendance) {
            ...
            if(selectedItem == null) {
                ...
                List<Present> attendances = new ArrayList<>();
                try {
                    long event_id = ((Event) selectedItem).getId();
                    attendances = eventAttendanceService.listPresentsByEvent(event_id);
                } catch(Exception ex) {
                    ex.printStackTrace();
                }

                CsvFileWriter csvFileWriter = new CsvFileWriter();
                String csv = csvFileWriter.writeCsvFile(getApplicationContext(), attendances);

                File downloadDir = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/report.csv");
